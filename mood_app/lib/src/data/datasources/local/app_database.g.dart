    ...
    final databaseOptions = sqflite.OpenDatabaseOptions(
      ...
      onCreate: (database, version) async {
        await database.execute(
            'CREATE TABLE IF NOT EXISTS `moods_table` (`id` INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, `name` TEXT NOT NULL, `pathToImg` TEXT NOT NULL, `date` INTEGER, `description` TEXT)');
...
class _$MoodDao extends MoodDao {
  ...
  Future<List<Mood>> getAllMoods() async {
    return _queryAdapter.queryList('SELECT * FROM moods_table',
        mapper: (Map<String, Object?> row) => Mood(
            id: row['id'] as int,
            name: row['name'] as String,
            pathToImg: row['pathToImg'] as String,
  ...
  Future<void> insertMood(Mood mood) async {
    await _moodInsertionAdapter.insert(mood, OnConflictStrategy.replace);
  }
}
